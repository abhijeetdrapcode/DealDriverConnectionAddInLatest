...
let dealSelect, sendDealButton, fetchDataButton, fetchWordData;
...
function initPostLoginElements() {
  ...
  //   const fetchWordDataButton = document.getElementById("fetchWordData");
  //   if (fetchWordDataButton) {
  //     fetchWordDataButton.addEventListener("click", getWordData);
  //   }
  // });

  if (fetchWordDataButton) {
    if (fetchWordDataHandler) {
...
async function sendCategoryData(
  ...
  try {
    ...
    if (response.ok) {
      ...
      const action = category === "postClosing" ? "Post-closing" : "Closing";
      showMessage(`${action} ${category} data sent successfully to ${dealName}`);
    ...
    console.error(`Error in sendCategoryData (${category}):`, error);
...
async function sendRepresentationData(
  ...
  const dealTenantId = JSON.parse(localStorage.getItem("loginResponseData"));
  console.log(
    "This is the login Response data from the sendRepresentation Data: ",
    dealTenantId.userDetails.tenantId
  );

  let tenants = dealTenantId.userDetails.tenantId;
  console.log("This is the deal uuid: ", dealUuid);

  const matchedTenant = tenants.find((tenant) => tenant.deal?.some((d) => d.uuid === dealUuid));

  const tenId = matchedTenant ? matchedTenant.uuid : null;

  console.log("Matched Tenant ID:", tenId);
  console.log("This is the matched deal: ", matchedTenant);
  ...
  try {
    ...
    console.log("This is the tenantId in representation data: ", tenId);
...
async function getWordData() {
  const fetchWordDataButton = document.getElementById("fetchWordData");
  const originalButtonText = fetchWordDataButton.textContent;
  const environment = localStorage.getItem("selectedEnvironment");

  try {
    // Disable the button immediately
    fetchWordDataButton.disabled = true;
    fetchWordDataButton.textContent = "Fetching...";
    fetchWordDataButton.style.opacity = "0.6";
    fetchWordDataButton.style.cursor = "not-allowed";
    ...
    console.log("Code reached getWordData()");
    ...
    const response = await fetch("https://dealdriverapi.drapcode.co/getDealDriverData", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    // Track if any valid items were processed
    ...
    await Word.run(async (context) => {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        // Check if the item is valid and contains the pipe separator
        if (typeof rawText === "string" && rawText.trim() && rawText.includes("|")) {
          const parts = rawText
            .split("|")
            .map((p) => p.trim())
            .filter(Boolean);

          // Only process if we have valid parts
          if (parts.length > 0) {
            for (let j = 0; j < parts.length; j++) {
              if (j % 2 === 0) {
                // Even index → Heading
                const heading = body.insertParagraph(parts[j], Word.InsertLocation.end);
                heading.styleBuiltIn = Word.Style.heading1;
                heading.font.bold = true;
                heading.alignment = Word.Alignment.left;
              } else {
                // Odd index → Paragraph (justified)
                const paragraph = body.insertParagraph(parts[j], Word.InsertLocation.end);
      ...
      // Only show success message if items were actually processed
      if (itemsProcessed > 0) {
    ...
    showMessage("Error fetching deal data: " + error.message, true);
  } finally {
    // Re-enable the button regardless of success or failure
    fetchWordDataButton.disabled = false;
